  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose} modal={true}>
      <DialogContent 
        ...
        dir="rtl"
      >
        <DialogHeader className="px-4 py-2.5 border-b border-slate-200 flex-shrink-0 bg-slate-50">
          <div>
        ...
        <div className="flex-1 overflow-hidden px-4 py-2.5">
          {isLoading ? (
            ...
            <div className="grid grid-cols-2 gap-2.5 text-xs h-full overflow-y-auto" style={{ maxHeight: 'calc(85vh - 80px)' }}>
              {/* Description */}
              {snapshot.description && (
                <div className="bg-slate-50 rounded-md p-2 border border-slate-200">
                  <div className="flex items-center gap-1.5 mb-1">
                    <FileText className="h-3 w-3 text-slate-600" />
                    <h3 className="text-xs font-bold text-slate-900">תיאור</h3>
                  </div>
                  <p className="text-[11px] text-slate-700 leading-relaxed">{snapshot.description}</p>
                </div>
              )}
              ...
              <div className="bg-blue-50 rounded-md p-2 border border-blue-200">
                <div className="flex items-center gap-1.5 mb-1.5">
                  ...
                  <h3 className="text-xs font-bold text-slate-900">תוכנית אימונים</h3>
                </div>
              ...
              <div className="bg-red-50 rounded-md p-2 border border-red-200">
                <div className="flex items-center gap-1.5 mb-1.5">
                  ...
                  <h3 className="text-xs font-bold text-slate-900">אימון אירובי</h3>
                </div>
                {snapshot.cardio_training && Array.isArray(snapshot.cardio_training) && snapshot.cardio_training.length > 0 ? (
                  ...
                  <p className="text-[11px] text-slate-500">אין אימונים אירוביים</p>
                )}
              </div>
              ...
              <div className="bg-yellow-50 rounded-md p-2 border border-yellow-200">
                <div className="flex items-center gap-1.5 mb-1.5">
                  ...
                  <h3 className="text-xs font-bold text-slate-900">אימון אינטרוולים</h3>
                </div>
                {snapshot.interval_training && Array.isArray(snapshot.interval_training) && snapshot.interval_training.length > 0 ? (
                  ...
                  <p className="text-[11px] text-slate-500">אין אימוני אינטרוולים</p>
                )}
              </div>
              ...
              <div className="bg-green-50 rounded-md p-2 border border-green-200">
                <div className="flex items-center gap-1.5 mb-1.5">
                  ...
                  <h3 className="text-xs font-bold text-slate-900">תזונה</h3>
                </div>
                {snapshot.nutrition_targets || snapshot.nutrition_template ? (
                  <div className="space-y-1">
                    {snapshot.nutrition_template && (
                      <div className="text-[11px]">
                        <span className="text-[10px] font-semibold text-slate-500">תבנית:</span>
                        <span className="font-semibold text-slate-900 mr-1">{snapshot.nutrition_template.name}</span>
                      </div>
                    )}
                  ...
                  <p className="text-[11px] text-slate-500">אין יעדי תזונה</p>
                )}
              </div>
              ...
              <div className="bg-purple-50 rounded-md p-2 border border-purple-200">
                <div className="flex items-center gap-1.5 mb-1.5">
                  ...
                  <h3 className="text-xs font-bold text-slate-900">תוספים</h3>
                </div>
                {snapshot.supplements && Array.isArray(snapshot.supplements) && snapshot.supplements.length > 0 ? (
                  ...
                  <p className="text-[11px] text-slate-500">אין תוספים</p>
                )}
              </div>

              {/* Eating Guidelines - Moved to left column under supplements */}
              {(snapshot.eating_order || snapshot.eating_rules) && (
                <div className="bg-slate-50 rounded-md p-2 border border-slate-200">
                  <div className="flex items-center gap-1.5 mb-1.5">
                    <FileText className="h-3 w-3 text-slate-600" />
                    <h3 className="text-xs font-bold text-slate-900">הנחיות אכילה</h3>
                  </div>
                  <div className="space-y-1">
                    {snapshot.eating_order && (
                      <div className="text-[11px] text-slate-700">
                        <span className="text-[10px] font-semibold text-slate-500">סדר אכילה:</span>
                        <span className="mr-1">{snapshot.eating_order}</span>
                      </div>
                    )}
                    {snapshot.eating_rules && (
                      <div className="text-[11px] text-slate-700">
                        <span className="text-[10px] font-semibold text-slate-500">כללי אכילה:</span>
                        <span className="mr-1">{snapshot.eating_rules}</span>
                      </div>
                    )}
                  </div>
                </div>
              ...
              <div className="bg-cyan-50 rounded-md p-2 border border-cyan-200">
                <div className="flex items-center gap-1.5 mb-1.5">
                  ...
                  <h3 className="text-xs font-bold text-slate-900">יעד צעדים</h3>
                </div>
                {snapshot.steps_goal ? (
                  <div className="space-y-0.5">
                    <div className="text-[11px] text-slate-700">
                    ...
                    {snapshot.steps_instructions && (
                      <div className="text-[11px] text-slate-600">
                        <span className="text-[10px] font-semibold text-slate-500">הוראות:</span>
                        <span className="mr-1">{snapshot.steps_instructions}</span>
                      </div>
                    )}
                  </div>
                ) : (
                  <p className="text-[11px] text-slate-500">אין יעד צעדים</p>
                )}
              </div>
            </div>
          )}
