/**
 * PersonalCheckInSettingsDialog Component
 * 
 * Dialog for setting personalized check-in settings for a specific customer/lead
 * Reuses the same components from CheckInSettingsPage
 */
...
interface PersonalCheckInSettingsDialogProps {
  isOpen: boolean;
  onClose: () => void;
...
export const PersonalCheckInSettingsDialog: React.FC<PersonalCheckInSettingsDialogProps> = ({
  isOpen,
  onClose,
  customerId,
  customerName,
}) => {
  const { configuration, isLoading, saveConfiguration } = useCheckInFieldConfigurations(customerId);
      ...
      const configWithUnits: CheckInFieldConfiguration = {
        ...
        fields: Object.fromEntries(
          Object.entries(configuration.fields).map(([key, field]) => [
            ...
            {
              ...field,
              unit: field.unit || AVAILABLE_UNITS[0],
            },
          ])
        ),
      };
  ...
  const handleSave = async () => {
    try {
      await saveConfiguration(localConfig, false); // Save as customer-specific config
      setHasChanges(false);
      toast({
        title: 'הצלחה',
        description: 'הגדרות צ\'ק-אין מותאמות אישית נשמרו בהצלחה',
      });
      onClose();
    } catch (error: any) {
      toast({
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose} dir="rtl">
      <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto" dir="rtl">
        <DialogHeader>
          <DialogTitle>הגדרות צ'ק-אין מותאמות אישית</DialogTitle>
          <DialogDescription>
            {customerName && `הגדרות מותאמות אישית עבור ${customerName}`}
            {!customerName && 'הגדרות מותאמות אישית עבור לקוח זה'}
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-4 py-4">
            <div className="flex items-center justify-between">
              <p className="text-sm text-slate-600">
                התאם אישית אילו שדות וסעיפים יוצגו ללקוח זה בטופס הצ'ק-אין היומי
              </p>
